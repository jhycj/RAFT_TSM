...
class Flow_refinement(nn.Module):
    def __init__(self, num_segments, expansion = 1, pos=2):
        ...
        self.W = nn.Parameter(tr.randn(2).softmax(-1))

        # Channelwise Weighted Sum 
        #self.Channel_W_x = nn.Parameter(tr.randn(self.out_channel, self.out_channel).softmax(-1)) 
        #self.Channel_W_res = nn.Parameter(tr.randn(self.out_channel, self.out_channel).softmax(-1)) 
    ...
    def forward(self, x, res, match_v): # x<-flow (u,v), res<-x, match_v : max_value에 해당 -> (B*(T-1), k, H, W), k=1  
        ...
        x = self.conv3(x)
        x = self.conv4(x) 
        #print(f'x.shape: {x.shape}') # x.shape: torch.Size([16, 512, 28, 28])
        
        #W = self.W.softmax(-1)        
     
        x = self.W[0]*x + self.W[1]*res 

        #x = tr.matmul(self.Channel_W_x, x) + tr.matmul(self.Channel_W_res, res) 
        #print(x.shape) 
...
def resnet18(pretrained=False, shift='TSM',num_segments = 8, flow_estimation=0, **kwargs):
    ...
    if pretrained:
        ...
        import torch 
        pretrained_dict = torch.load('./pretrained/resnet18-5c106cde.pth')
